...
import {
  deleteInvoice,
  listInvoiceByGuestName,
  listStayingAndComingInvoices,
} from "../db/invoice";
...
export const InvoiceManager = (props: InvoiceManagerProps) => {
  ...
  const confirmDeletion = async () => {
    try {
      if (
        deletingInv === undefined ||
        deletingInv === null ||
        deletingInv.id === undefined
      ) {
        return;
      }
      ...
      if (rsp.status === 200) {
        ...
        console.error(
          "Failed to delete invoice %s: status %s",
          deletingInv.id,
          rsp.status,
        );
      }
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              {deletingInv !== null && deletingInv !== undefined
                ? "Delete invoice of " + deletingInv.guestName + " ?"
                : "You need to choose the invoice to delete"}
            </p>
          </div>
        </Modal.Body>
