...
export const TaxableInvoiceManager = (props: TaxableInvoiceManagerProps) => {
    ...
    const calculateTax = async () => {
        ...
        setError(undefined);
        try {
            const response = await calculateTaxableInvoices(fromDate, toDate, !preview);
            if (response.status === 401 || response.status === 403) {
                props.handleUnauthorized();
                return;
            }
            if (response.status !== 200) throw new Error(`Tax calculation API returned ${response.status}`);
            if (!preview) {
                await fetchInvoices();
            } else {
                setCalculationResults(Array.isArray(response.data) ? response.data : []);
                setShowCalculationResults(true);
            }
        } catch (e) {
            console.error("Error while calculating taxable invoices", e);
            setError("Unable to calculate invoice tax.");
        } finally {
    ...
    return (
        <div className="relative flex h-[calc(100dvh-3rem)] flex-col">
            ...
            <Modal show={showCalculationResults} onClose={() => setShowCalculationResults(false)}>
                <Modal.Header>Calculated taxable invoices</Modal.Header>
                <Modal.Body>
                    {calculationResults.length === 0 ? <div className="text-gray-500">No calculated invoices found.</div> : <div className="divide-y">
                        {calculationResults.map((calculation, index) => {
                            const invoice = calculation.result?.invoice;
                            return <div key={invoice?.id || index} className="flex items-center justify-between py-2">
                                <span className="text-sm text-gray-700">{invoice && dateValue(invoice) ? new Date(dateValue(invoice)).toLocaleDateString() : "-"}</span>
                                <span className="flex-1 px-3 font-medium text-green-900">{invoice ? guestValue(invoice) : "-"}</span>
                                <span className="font-mono text-sm">{invoice ? formatVND(subtotalValue(invoice)) : "-"}</span>
                            </div>;
                        })}
                    </div>}
                </Modal.Body>
                <Modal.Footer><Button color="gray" onClick={() => setShowCalculationResults(false)}>Close</Button></Modal.Footer>
            </Modal>
